...
const DEV_PHONE_OTP_BYPASS = true
const TEST_OTP = '0000'
...
export function StepMobileVerification({ data, onPrev, onSubmit, isSubmitting }: Props) {
  ...
  async function handleSendOtp() {
    ...
    if (DEV_PHONE_OTP_BYPASS) {
      // Mock OTP sending for development/testing
      setTimeout(() => {
        setIsSending(false)
        setOtpSent(true)
        startCountdown()
        toast.success('OTP has been sent successfully (Test Mode).')
      }, 500)
      return
    }
  ...
  async function handleVerify() {
    const expectedOtpLength = DEV_PHONE_OTP_BYPASS ? TEST_OTP.length : 6
    ...
    if (DEV_PHONE_OTP_BYPASS) {
      setTimeout(() => {
        ...
        if (otp === TEST_OTP) {
          setVerified(true)
          data.phone = phone // Mutate data.phone so the wizard has access to it
          toast.success('Phone number verified successfully.')
          setTimeout(() => {
            onSubmit()
          }, 1200)
        } else {
          const msg = `Invalid code. For testing, please enter ${TEST_OTP}.`
          toast.error(msg)
          setOtpError(msg)
        }
      }, 500)
        ...
        {otpSent && (
          ...
          >
            ...
            <Input
              ...
              inputMode="numeric"
              pattern="\d*"
              placeholder={DEV_PHONE_OTP_BYPASS ? `${TEST_OTP.length}-digit OTP` : "6-digit OTP"}
              value={otp}
              maxLength={DEV_PHONE_OTP_BYPASS ? TEST_OTP.length : 6}
              autoFocus
              onChange={(e) => {
                const limit = DEV_PHONE_OTP_BYPASS ? TEST_OTP.length : 6
                setOtp(e.target.value.replace(/\D/g, '').slice(0, limit))
                setOtpError('')
              }}
            ...
            {DEV_PHONE_OTP_BYPASS && (
              <p className="text-xs text-amber-600 font-medium mt-1">
                Testing mode: currently the verification code for all numbers is {TEST_OTP}.
              </p>
            )}
            ...
            {!verified && (
              <div className="pt-3 flex flex-col gap-3">
                <Button
                  ...
                  disabled={otp.length !== (DEV_PHONE_OTP_BYPASS ? TEST_OTP.length : 6) || isVerifying}
                  variant="gold"
                  className="w-full"
                >
