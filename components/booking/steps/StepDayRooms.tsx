...
const ROOM_PRESETS = [5, 10, 25, 50, 100]
  ...
  return (
    ...
    >
      ...
      <p className="text-body text-[#737373] mb-10">
        Estimate the number of rooms needed for guests. Breakfast is included complimentary with every room.
      </p>
        ...
        <div className="rounded-3xl border border-[#E8E2D8] bg-white/80 backdrop-blur-md p-8 md:p-10 shadow-3d flex flex-col items-center gap-8">
          ...
          <div className="flex items-center gap-6">
            ...
            <button
              ...
              disabled={rooms <= 1}
              className={cn(
                'w-12 h-12 rounded-2xl border flex items-center justify-center transition-all duration-200 active:scale-90',
                rooms <= 1
                  ? 'border-[#E8E5E0] text-[#D4CFC9] cursor-not-allowed'
                  : 'border-[#E8D9A8] text-[#1A1A1A] hover:border-[#C5A85C] hover:bg-[#FDFCFA] hover:shadow-sm cursor-pointer'
              )}
            >
            ...
            <div className="w-28 h-28 rounded-2xl bg-gradient-to-br from-[#FAF6EE] to-[#F5EDD6] border border-[#E8D9A8] flex items-center justify-center overflow-hidden relative shadow-inner">
              <AnimatePresence mode="popLayout">
                <motion.span
                  ...
                  className="absolute text-5xl font-serif font-bold text-[#1A1A1A]"
                >
                  {rooms}
                </motion.span>
              </AnimatePresence>
          ...
          <p className="text-xs text-[#A8A8A8] text-center border-t border-[#F0EDE9] pt-4 w-full">
            {rooms} {rooms === 1 ? 'room' : 'rooms'} selected for this day.
          </p>
        </div>
      </Card3D>
